...
public class Play extends ListeningGameComponent
{
	...
	/**
	 * Methods mousePresses() through getGoals() are used to make a snarled/bulleted
	 * method look much cleaner
	 *
	 * mousePresses() is the methods that handles all of the mouse presses
	 */
	private void mousePresses() {
		if(isMousePressed(1))
		{
			if(!drag)
			{
	...
	//handles all the different key presses
	...
	//Handles the long inner code for when L is pressed
	private void pressL() {
		try
		{
			resetKeys();
	...
	//Handles the long inner code for when enter is pressed
	...
	//checks the movement controls keys during certain situations
	private void cometDrag() {
		if((comet != null) && (!drag))
		{
			if(isKeyPressed("UP"))
